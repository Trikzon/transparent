...
public abstract class EnderDragonRendererMixin extends EntityRenderer<EnderDragon> {
    ...
    @Redirect(method = "renderCrystalBeams", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/renderer/MultiBufferSource;getBuffer(Lnet/minecraft/client/renderer/RenderType;)Lcom/mojang/blaze3d/vertex/VertexConsumer;"))
    private static VertexConsumer redirectGetBufferInRenderCrystalBeams(MultiBufferSource buffer, RenderType renderType) {
        return Transparent.CONFIG.endCrystalBeam ?
                buffer.getBuffer(TransparentRenderTypes.entitySmoothCutout(
